...
<div class="container-fluid">
  <div class="fade-in">
    <div class="row">
      <div class="col-sm-12">
        <div class="card">
          <div class="card-header"><h4>{{ trans('wine.menu_element') }}</h4></div>
            <div class="card-body">
                <div class="row mb-3 ml-3">
                    <a class="btn btn-lg btn-primary" href="{{ route('menu.create') }}">{{ trans('wine.add_new') }}</a>
                </div>
                <div class="row mb-3">
                    <div class="col-sm-4">
                        <form action="{{ route('menu.index') }}" methos="GET">
                            ...
                            <br>
                            <button type="submit" class="btn btn-primary">{{ trans('wine.change') }}</button>
                        </form>
                    </div>
                </div>
<?php
    ...
    function renderDropdownForMenuEdit($data, $role){
        if(array_key_exists('slug', $data) && $data['slug'] === 'dropdown'){
            ...
            echo '</td>';
            echo '<td>';
            echo '<a class="btn btn-primary" href="' . route('menu.show', ['id' => $data['id']]) . '">'. trans('wine.view') .'</a>';
            echo '</td>';
            echo '<td>';
            echo '<a class="btn btn-primary" href="' . route('menu.edit', ['id' => $data['id']]) . '">'. trans('wine.edit') .'</a>';
            echo '</td>';
            echo '<td>';
            echo '<a class="btn btn-danger" href="' . route('menu.delete', ['id' => $data['id']]) . '">'. trans('wine.delete') .'</a>';
            echo '</td>';
            echo '</tr>';
            ...
            for($i = 0; $i < count($data); $i++){
                if( $data[$i]['slug'] === 'link' ){
                    ...
                    echo '</td>';
                    echo '<td>';
                    echo '<a class="btn btn-primary" href="' . route('menu.show', ['id' => $data[$i]['id']]) . '">'. trans('wine.view') .'</a>';
                    echo '</td>';
                    echo '<td>';
                    echo '<a class="btn btn-primary" href="' . route('menu.edit', ['id' => $data[$i]['id']]) . '">'. trans('wine.edit') .'</a>';
                    echo '</td>';
                    echo '<td>';
                    echo '<a class="btn btn-danger" href="' . route('menu.delete', ['id' => $data[$i]['id']]) . '">'. trans('wine.delete') .'</a>';
                    echo '</td>';
                    echo '</tr>';
              ...
              ?>
                ...
                <table class="table table-striped table-bordered datatable">
                    <thead>
                        <tr>
                            <th></th>
                            <th>{{ trans('wine.Type') }}</th>
                            <th>{{ trans('wine.Name') }}</th>
                            <th>{{ trans('wine.href') }}</th>
                            <th>{{ trans('wine.sequence') }}</th>
                            <th></th>
                            <th></th>
                            <th></th>
                            <th></th>
                ...
                @foreach($menuToEdit as $menuel)
                    @if($menuel['slug'] === 'link')
                        <tr>
                            ...
                            <td>
                                <a class="btn btn-primary" href="{{ route('menu.show', ['id' => $menuel['id']]) }}">{{ trans('wine.view') }}</a>
                            </td>
                            <td>
                                <a class="btn btn-primary" href="{{ route('menu.edit', ['id' => $menuel['id']]) }}">{{ trans('wine.edit') }}</a>
                            </td>
                            <td>
                                <a class="btn btn-danger" href="{{ route('menu.delete', ['id' => $menuel['id']]) }}">{{ trans('wine.delete') }}</a>
                            </td>
                        </tr>
